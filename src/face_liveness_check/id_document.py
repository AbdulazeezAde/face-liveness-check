...
class IdDocumentExtractor:
    ...
    def extract(
        ...
        *,
        ...
        image = load_reference_image_bgr(source) if isinstance(source, (str, Path)) else source
        ...
        warnings = tuple(dict.fromkeys((*normalized.quality.warnings, *template_warnings, *portrait_warnings)))
    ...
    def _select_template(self, blocks: Sequence[OcrTextBlock], requested: DocumentType) -> tuple[DocumentTemplate | None, DocumentType]:
        ...
        if len(_mrz_lines(blocks)) >= 2:
            return self.templates.get(DocumentType.PASSPORT_TD3), DocumentType.PASSPORT_TD3
        return None, DocumentType.UNKNOWN

    def _portrait(self, image_bgr: np.ndarray) -> tuple[np.ndarray | None, tuple[str, ...]]:
        if self.detector is None:
            return None, ("portrait extraction was not configured",)
